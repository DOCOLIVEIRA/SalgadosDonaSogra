...
function enviarWhatsApp() {
    ...
    // Enviar para o banco primeiro via API REST Laravel
    fetch('/api/pedidos', {
        method: 'POST',
        headers: { 
        ...
        body: JSON.stringify({
            ...
            itens: carrinho.map(item => ({
                produto_id: parseInt(item.id) || 1,
                quantidade: item.qty
            }))
        })
    }).then(r => r.json()).then(resp => {
        if (!resp.success && !resp.sucesso) {
            alert('Erro ao registrar pedido no sistema: ' + (resp.message || resp.erro || 'Desconhecido'));
        ...
        .then(data => {
            if (data.success || data.sucesso) {
                ...
                PRODUTOS = lista.map(p => ({
                    ...
                    nome: p.nome,
                    desc: p.descricao,
                    preco: parseFloat(p.preco_unitario || p.preco),
                    img: p.imagem || 'img/logo.png'
                }));
