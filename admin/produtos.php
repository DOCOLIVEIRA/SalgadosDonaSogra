...
$pdo = get_connection();
$stmt = $pdo->query("SELECT * FROM products ORDER BY nome ASC");
...
<div class="card">
    <div class="card-header">
        ...
        <span style="color:#555; font-size:0.82rem;">
            <?= count($produtos) ?> produto(s) cadastrado(s)
        </span>
    </div>
                    ...
                    <tr>
                        <td>
                            <div style="display:flex; align-items:center; gap:0.6rem;">
                                ...
                                <?php endif; ?>
                                <div>
                                    <div style="font-weight:700; color:#ddd;">
                        ...
                        <td style="color:#888; font-size:0.88rem;">
                            R$
                            <?= number_format($p['preco_unitario'] * 100, 2, ',', '.') ?>
                        </td>
                        <td>
                            <?php if ($p['quantidade_estoque'] <= 50): ?>
                                <span style="color:#e74c3c; font-weight:700;">⚠
                                    <?= $p['quantidade_estoque'] ?>
                                </span>
                            <?php elseif ($p['quantidade_estoque'] <= 150): ?>
                                <span style="color:#F0A500; font-weight:700;">●
                                    <?= $p['quantidade_estoque'] ?>
                                </span>
                            <?php else: ?>
                        ...
                        <!-- Alterar Estoque -->
                        <td>
                            <form method="POST" action="api/atualizar_estoque.php"
        ...
        <form method="POST" id="precoForm" action="api/alterar_preco.php">
            ...
            <div class="form-group">
                ...
                <input type="number" name="novo_preco" id="novoPrecoInput" step="0.01" min="0.01" class="form-input"
                    style="width:100%" placeholder="0.00" required />
            ...
            <div
                style="background:rgba(240,165,0,0.08); border:1px solid rgba(240,165,0,0.2); border-radius:8px; padding:0.75rem; margin-bottom:1.25rem;">
